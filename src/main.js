...
if (els.runTraceButton && els.traceOutput) {
  els.runTraceButton.addEventListener('click', () => {
    ...
    els.traceOutput.textContent =
      ...
      `\n${switchLine}` +
      `\n${coreModLine}` +
      `\nfeedback: consumption (fatigue) modulated by step durIdx; carry fraction of prior continuity + stress decay rate now explicitly scaled by durability (high-dur retains more coherence memory / sheds history stress faster); reinf/debt amplified (closed full-loop at accumulator level); path memory (accum carry * (1-stress)) now feeds forward to boost next-step route and raise next scatter (history state changes the inputs to the 4-bucket + grammar + coherence calc); memoryMod passed to core calculateOutcome (history inertia now also directly boosts coherence/identity and relaxes the preservation gate in the single-step view, like durability); memory now further eases fatigue and amplifies reinf/debt (stronger virtuous cycles when inertia high); adaptive commitment value boosted by current memory (high inertia makes sticking with a regime more attractive in lookahead); memory now modulates the 4-bucket outcomes in core (high inertia shifts admitted+stored up, scattered down — history changes the immediate fate fractions); memory now explicitly modulates grammarAlignment and coherenceMetric in core (inertia as a direct grammar factor strengthening alignment); memory now directly scales accumulator carry retention and stress decay (higher memory retains more coherence carry for deeper persistence across steps and sheds stress faster); the trace summary now includes memoryAdjustedFinalIdentity (final identity quality boosted by avgMem, so preservation is stronger in quality not just binary); the whole trace can be memory-carried preserved if avgMem high even if last step marginal (memoryCarriedPres flag); when carried, the carried quality is memoryCarriedFinalIdentity (the memory-adjusted final id as the carried strength); adaptive choice now blends MC expected preservation (non-myopic averaging over random future condition sequences). PathQuality (new running accumulator: per-step blend of preservation success * durability * inertia) now explicitly scales carryFactor + stressRetain beyond dur+mem, dynamically lowers the memory-rescuer threshold (high quality histories rescue marginal steps more easily), and makes the memoryCarriedFinalPres quality gate itself adaptive (high avgPathQuality lowers the required carried final identity for the whole trace to count as quality-carried-preserved). Adaptive policy value scoring quality-weights its non-myopic fut+MC terms by current pathQ estimate. Trace steps now surface pathQuality; summary adds avgPathQuality, finalPathQuality, memoryCarriedFinalPresQualityGate (the effective dynamic threshold used). PathQuality now also directly eases per-step fatigue/consumption and further amplifies reinf/debt scales (high sustained quality makes coherence *cheaper* to maintain over time and good policy choices self-amplify more strongly; breaking a high-quality streak incurs stronger debt). PathQuality (running) is now also passed into core calculateOutcome for the current step: high streak quality directly boosts coherence/identity numbers and relaxes the per-step preservation gate (history quality momentum now affects the immediate test, parallel to pathMemory). Adaptive policy now includes explicit "projected pathQuality at end of remaining horizon" term in its per-step value (deeper non-myopic quality-seeking: prefers regimes that not only preserve but leave the history in a high-quality end state). Trace summary now also exposes pathQBoostedFinalIdentity (avgPathQ across the trace gives a quality lift to the ending identity); high avgPathQ can now rescue overall trace-level finalPreserved for a marginal last step (history quality as a whole carries the preservation verdict). New: pathQBoostedPreserved flag (true when the pathQ boost was the mechanism that set finalPreserved); this gives a clean parallel to memoryCarriedPreserved for quality-driven whole-trace rescue. Commitment lookahead in policy now uses pathQBoostedFinalIdentity (when present) for the preservation value under sticking with a regime — non-myopic choice is now aware of the quality-boosted rescue.` +
