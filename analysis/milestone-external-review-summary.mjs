...
const unificationMap = [
  ...
  },
  {
    ...
    currentReading:
      'Shows held-out material ordering and exact composition accounting. The measured refractive-index challenge is now explicit and unresolved: a first-pass topology-only optical-property proxy exists but does not clear both measured targets.',
  },
];
...
This challenge anchors measured SiO2 and Na2SiO3 refractive-index targets and now includes a first-pass topology-only refractive-index proxy. It intentionally remains unresolved until that predictor clears both measured targets without endpoint fitting. It prevents NBO/T composition accounting, or a weak proxy, from being mistaken for a measured material-property pass.
...
- Resolve the material refractive-index challenge by improving the predeclared topology-only optical-property predictor until it clears measured SiO2/Na2SiO3 tolerance without endpoint fitting.
