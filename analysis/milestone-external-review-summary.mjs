...
const benchmarkFiles = [
  ...
  'external-em-boundary-propagation-comparator.json',
  'external-em-oblique-fresnel-comparator.json',
...
const unificationMap = [
  ...
  },
  {
    ...
    currentReading:
      'Shows held-out material ordering and exact composition accounting. The measured refractive-index challenge is now explicit and unresolved: a first-pass topology-only proxy fails, the target-implied slope/framework-Al repair candidate fails fresh anorthite validation, and a second-generation modifier-identity candidate remains quarantined pending a new held-out target.',
  },
];
...
This challenge anchors measured SiO2, Na2SiO3, held-out NaAlSi3O8, and fresh-validation CaAl2Si2O8 refractive-index targets and now includes a first-pass topology-only refractive-index proxy. It also records a target-implied slope/framework-Al repair candidate, but the candidate fails fresh anorthite validation. A second-generation modifier-identity candidate now fits the current rows by adding a divalent-modifier correction, but it is target-informed after the anorthite miss and is not counted as a pass. This prevents NBO/T composition accounting, or a target-fitted proxy, from being mistaken for a measured material-property pass.
